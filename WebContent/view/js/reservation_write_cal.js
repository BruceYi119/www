...
//	year.find(`option[value=${y}]`).prop('selected', true);		// option 중 현재 년도를 찾아서, select 시켜놓기
//	month.find(`option[value=${m}]`).prop('selected', true);	// option 중 현재 월을 찾아서, select 시켜놓기
};
// 달력을 그리는 함수
const drawCalendar = (date = new Date()) => {
	const tbody = $('#cal tbody');								// 달력 그려지는 태그
	const y = date.getFullYear();								// 0000년 부터 시작하는 이번 년도
	const m = date.getMonth();									// 오늘의 월
