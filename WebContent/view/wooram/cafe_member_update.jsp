...
<%
	...
	String sql = "select * from cmember where cmno = ?";
	dao.select(sql, cmno);
	...
	out.print(cmno);
	out.print(dao.getDto().getNick().toString() + dao.getDto().getReason().toString()
			+ dao.getDto().getPobu().toString());
%>
<!DOCTYPE html>
<html>
<head>
	...
	<div id="wrap">
		...
		<div class="base_wrap">
			<div id="container" align="center">
				...
				<!-- 여기까지  -->
				<div id="in_con">
					<form method="post" action="cafe_member_update_ok.jsp"
						onsubmit="return formUpCheck(this);" name="forms">
						<input type="hidden" name="cmno" value="<%=cmno%>" /> <input
							type="hidden" name="n_chk" id="n_chk" value=0 />
						<table align="center">
							<tr>
								<td>카페 활동명</td>
								<td><input type="text" name="nick"
									onkeyup="nick_check(this.value)" maxlength="10"
									value="<%=dao.getDto().getNick()%>" style="border: 0px;"
									readonly="readonly"><span id="unick"></span></td>
							</tr>
							<tr>
								<td>가입 사유</td>
								<td><input type="text" name="reason"
									value="<%=dao.getDto().getReason()%>"></td>
							</tr>
							<tr>
								<td>가입 인사</td>
								<td><textarea name="pobu"><%=dao.getDto().getPobu()%></textarea></td>
							</tr>
							<tr align="center">
